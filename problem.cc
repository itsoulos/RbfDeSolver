...
void	Problem::getSample(Matrix &x)
{
	if(has_margins==(MARGINS_LEFT|MARGINS_RIGHT))
	{
		for(int i=0;i<dimension;i++) 
		{
			double l=fabs(lmargin[i])<1?lmargin[i]:-1;
			double r=rmargin[i]<1?rmargin[i]:1;
			//x[i]=l+(r-l)*drand48();
...
double	Problem::getGradientCriterion(Matrix x1,Matrix x2,Matrix &g1,Matrix &g2)
{
	double s=0;
	for(int i=0;i<x1.size();i++)
	{
