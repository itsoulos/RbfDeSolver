...
GPdeProgram::GPdeProgram(Rbf *m,double X0,double X1,double Y0,double Y1,int n,int b)
    :GProgram(m)
{
...
GPdeProgram::GPdeProgram(Rbf *m,QString filename)
    :GProgram(m)
{
...
double GPdeProgram::fitness(vector<double> &genome)
{
	 ...
	 * */
		...
		if(fg0==NULL) val1=g0(X); else val1=fg0(&X);
                penalty3+=pow(rbf->getValue(x)-val1,2.0);

                x[1]=y1;
		if(fg1==NULL) val2=g1(X); else val2=fg1(&X);
                penalty4+=pow(rbf->getValue(x)-val2,2.0);
 	...
 	for(y=y0;y<=y1;y+=stepy)
		...
		if(ff0==NULL) val3=f0(y); else val3=ff0(&y);
                penalty1+=pow(rbf->getValue(x)-val3,2.0);

                x[0]=x1;
		if(ff1==NULL) val4=f1(y); else val4=ff1(&y);
                penalty2+=pow(rbf->getValue(x)-val4,2.0);
	 ...
	 * */
        ...
        for(Y=y0;Y<y1+0.01;Y+=stepy)
        {
                for(X=x0;X<x1+0.01;X+=stepx)
                {
                        x[0]=X;
                        x[1]=Y;
                        double valx2= rbf->getSecondDerivative(x,0);

                        double valy2= rbf->getSecondDerivative(x,1);

                        double valx1= rbf->getDerivative(x,0);
                        double valy1= rbf->getDerivative(x,1);
                        double val=rbf->getValue(x);
			if(fpde==NULL)
